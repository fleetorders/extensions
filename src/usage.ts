...
// {claudeAiOauth:{accessToken,expiresAt,...}}. Bridgy is a READ-ONLY consumer
...
// bridgy), rotating it logs the CLI out. So bridgy reads, uses the token if
...
// means bridgy uses the SAME user the CLI does, even if a stray second entry
// (e.g. a former guarded login) is present under the same service.
...
// access token and its expiry, NOT the refresh token (bridgy never refreshes).
// Targets the CLI's canonical account first, then any entry under the service,
// so bridgy uses the same user the CLI does. Best-effort + bounded so a Keychain
...
async function readClaudeCredentials(): Promise<{ accessToken: string; expiresAt: number } | null> {
...
async function fetchAnthropicUsageLive(): Promise<ProviderUsage | null> {
  ...
  if (cred.expiresAt <= Date.now() + TOKEN_SKEW_MS) {
    dbg("stored access token expired — deferring to the CLI's next refresh; tee this cycle")
    return null
  }
  let res: Response
  try {
